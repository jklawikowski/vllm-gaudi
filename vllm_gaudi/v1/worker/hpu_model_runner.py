...
class HPUModelRunner(HpuKVConnectorModelRunnerMixin):
    ...
    def _get_mm_dummy_batch(
        ...
        if modality == 'image':
            ...
            # Audio modality: dummy data is generated by the MultiModalRegistry
            # based on the model config; no vision-specific options needed.
            pass
        else:
            logger.warning("Modality '%s' is not supported for HPU multimodal warmup, skipping.", modality)
            return None
    ...
    def warmup_multimodal_graphs(self, buckets):
        ...
        is_audio_warmup = (mm_config is not None and mm_config.get_limit_per_prompt("audio") is not None
                           and "audio" in self.mm_budget.mm_limits and self.mm_budget.mm_limits['audio'] != 0)
        warmup_configs = {
        ...
        for modality, max_items in self.mm_budget.mm_limits.items():
            if modality == 'image' and not is_image_warmup or modality == 'video' \
                    and not is_video_warmup or modality == 'audio' and not is_audio_warmup:
                continue
            phase = f'Graph/Multimodal({modality})'
            if modality == 'audio':
                # Audio modality: single warmup call, no vision-bucket iteration.
                count = 1
                mm_options = mm_config.get_limit_per_prompt(modality)
                if mm_options and hasattr(mm_options, 'count'):
                    count = mm_options.count
                batched_dummy_mm_inputs = self._get_mm_dummy_batch(modality,
                                                                   image_args=count,
                                                                   width=0,
                                                                   height=0)
                if batched_dummy_mm_inputs is not None:
                    self.model.embed_multimodal(**batched_dummy_mm_inputs)
                    self.log_warmup_multimodal(phase, 0, 1, count, 0, 0, 0)
                continue
    ...
    def warmup_model(self) -> None:
        ...
        if self.unified_attn:
            ...
            if not self.is_pooling_model and not self.num_mamba_layers \
